    ...
    <div className="min-h-screen bg-background pb-24">
      ...
      <div className="max-w-lg mx-auto px-6 pt-8">
        <div className="flex items-center justify-between mb-8">
          <div>
            <Logo size="sm" />
            <p className="text-sm text-muted-foreground mt-1">{t('hi')} <span className="font-semibold text-foreground">{user.name}</span> 👋</p>
          </div>
        </div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
          ...
          <div className="relative z-10">
            <p className="text-xs font-medium opacity-80 uppercase tracking-wider">{t('todaysExercise')}</p>
            ...
            <Button variant="secondary" className="mt-4 rounded-xl shadow-md" onClick={() => setActiveExercise(recommended)}>
              <Play className="w-4 h-4 mr-2" />{t('startExercise')}
            </Button>
          </div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="mb-6">
          <div className="flex items-center justify-between mb-3">
            <h3 className="font-semibold text-foreground flex items-center gap-2">
              <TrendingUp className="w-4 h-4 text-primary" />{t('yourProgress')}
            </h3>
            <span className="text-xs text-muted-foreground">{completedCount}/{totalExercises} {t('exercises')}</span>
          </div>
            ...
            {levels.map(level => {
              ...
              return (
                <div key={level} className="flex items-center gap-3">
                  <span className="text-sm w-24">{lvl.emoji} {t(`level${level.charAt(0).toUpperCase() + level.slice(1)}` as any)}</span>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="p-4 rounded-2xl bg-card border border-border mb-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <Bell className="w-5 h-5 text-primary" />
              <div>
                <p className="text-sm font-semibold text-foreground">{t('reminders')}</p>
                <p className="text-xs text-muted-foreground">{user.reminderTimes.join(' · ')}</p>
              </div>
            </div>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }} className="p-5 rounded-2xl bg-secondary/50 text-center">
          <p className="font-display italic text-foreground text-sm leading-relaxed">{todayQuote}</p>
        </motion.div>

        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="mt-6">
          <Button variant="outline" className="w-full h-14 rounded-2xl justify-between" onClick={() => navigate('/library')}>
            <span>{t('viewAllVideos')}</span><ChevronRight className="w-5 h-5" />
          </Button>
        </motion.div>
      </div>
      <BottomNav />
