...
import { exercises, levelLabels, goalLabels } from '@/data/exercises';
...
export default function VideoLibrary() {
  ...
  const filtered = exercises.filter(e => e.level === activeLevel);
    ...
    <div className="min-h-screen bg-background pb-24">
      ...
      <div className="max-w-lg mx-auto px-6 pt-8">
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
          <h1 className="text-2xl font-display font-bold text-foreground mb-1">Video-bibliotek</h1>
          <p className="text-sm text-muted-foreground mb-6">Alle øvelser – vælg dit niveau</p>
        </motion.div>
            ...
            {(['beginner', 'intermediate', 'advanced'] as const).map(level => {
              ...
              return (
                <TabsTrigger
                  key={level}
                  value={level}
                  className="flex-1 rounded-xl text-xs font-semibold data-[state=active]:bg-card data-[state=active]:shadow-sm"
                >
                  {lvl.emoji} {lvl.label}
                </TabsTrigger>
              );
